 ...
 * when the game is coming out. Ticks politely, then glitches.
 */
export default function BrokenCountdown() {
  ...
  const [vals, setVals] = useState<[number, number, number, number]>([247, 13, 37, 52]);
  const [scr, setScr] = useState<(string | null)[]>([null, null, null, null]);
  const [jitterCell, setJitterCell] = useState<number | null>(null);
      ...
      scheduleId = window.setTimeout(() => {
        ...
        scrambleId = window.setInterval(() => {
          ...
          if (n >= 9) {
            ...
            setJitterCell(null);
            setStatus(lost ? "status: lost track of time (relatable)" : "status: ticking… probably");
            if (alive) schedule();
          }
        }, 80);
